...
async function run() {
  try {
    ...
    const [categories] = await connection.query('SELECT id, name FROM categories WHERE slug = ? OR name LIKE ?', ['hd-closures', '%HD Closures%']);
    ...
    const [products] = await connection.query('SELECT id, name FROM products WHERE category_id = ?', [categoryId]);
